---
title: "Manipulação de dados (II) e programação básica em R"
subtitle: "NT265/NE441 - Análise de dados e apresentação de gráficos usando a linguagem R"
...
![](img/culmen_depth.png){.absolute width="600" height="400" right="-100" top="300"}
...
# funcao sum nas colunas (MARGIN = 2)
m_sum_col <- apply(m, 2, sum)
m_sum_col
```

## Família apply {.smaller}
...
```{r eval=FALSE}
apply(X, MARGIN, FUN, ...) # funcao sobre linhas ou colunas de uma matriz ou array
lapply(X, FUN, ...) # funcao a cada elemento de uma lista ou vetor, retornando sempre uma lista
sapply(X, FUN, ...) # similar a lapply, retornando matriz, vetor ou lista
vapply(X, FUN, FUN.VALUE, ...) # similar a sapply, mas define o tipo de retorno
tapply(X, INDEX, FUN, ...) # funcao sobre subsets de um vetor definido por fatores
mapply(FUN, ..., MoreArgs = NULL) # funcao a múltiplos vetores ou listas de forma paralela 
rapply(object, f, classes = "ANY", how = "replace") # funcao recursivamente a uma lista aninhada
```

- **Exemplos**:
```{r}
x <- list(a = 1:5, b = 6:10)
lapply(x, mean)  # media de cada lista
```

```{r}
tapply(1:10, rep(1:2, each = 5), sum)  # soma por grupo
```
...
![](img/if.png){.absolute right="-50" top="300"}
